...
//
////找到最近相交的一个点，储存相关信息
//bool SceneInteract(const Vector3& GetOrigin, const Vector3& dir, std::vector<Sphere>& spheres, RayHitInfo& hitInfo, Material& material)
//{
//	float spheresDistance = std::numeric_limits<float>::max();
//	float currentDist;
//	for (size_t i = 0; i < spheres.size(); ++i)
//	{
//		if (spheres[i].RayIntersect(GetOrigin, dir, currentDist) && currentDist < spheresDistance)
//		{
//			spheresDistance = currentDist;
//			hitInfo.HitPosition = GetOrigin + currentDist * dir;
...
//}
//
//Vector3 CastRay(const Vector3& GetOrigin, const Vector3& dir,std::vector<Sphere>& spheres,const std::vector<Light>& lights,int& recursiveDepth)
//{
//	RayHitInfo hit_info;
//	Material material;
//	//没有交点 返回背景颜色
//	if(recursiveDepth > MAX_RECURSIVE_COUNT || !SceneInteract(GetOrigin, dir, spheres, hit_info, material))
//	{
...
//	//相机参数
//	Vector3 GetOrigin = Vector3(0, 0, 0); 
...
//			Vector3 c = CastRay(GetOrigin, (pixelInWorldPos - GetOrigin).normalize(), spheres, lights,recursiveCount);
